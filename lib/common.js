        ...
        function Scroller(options) {
            ...
            function closestItem($column, top) {
                var index = $column.index(),
                    ...
                    total = items.length,
                    pos = Math.round((unitHeight * 2 - top) / 30),
                    i,
                    itemIndex = 0,
                    $item;
                if (pos < 0) pos = 0;
        ...
        Scroller.prototype = {
            init: function () {
                ...
                this._content._overlay.find('.scroller-column').each(function () {
                    $column = $(this);
                    $column.find('.scroller-column-items').css('top', unitHeight * 2 + 'px')
                        .find('.scroller-item.col-red').removeClass('col-red');
                    that.finishScroll($column);
                });
            },
            disable: function (column, indexes) {
                var $column = this._content._overlay.find('.scroller-column:nth-child(' + column + ')'),
                    $items = $column.find('.scroller-column-items'),
                    closest,
                    i,
                    top = parseInt($items.css('top'), 10);
                ...
                for (i = 0; i < indexes.length; i++) {
                    ...
                    $items.find('.scroller-item:nth-child(' + (indexes[i] + i) + ')').addClass('disabled');
                }
                closest = this.closestItem($column, top);
                $items.css('top', ((-closest[0] + 2) * unitHeight) + 'px');
                this.finishScroll($column);
                return this;
            },
            enable: function (column, indexes) {
                ...
                for (i = 0; i < indexes.length; i++) {
                    ...
                    $items.find('.scroller-item:nth-child(' + (indexes[i] + i) + ')').removeClass('disabled');
                }
                return this;
            },
            show: function() {
